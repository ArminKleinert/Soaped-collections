...
data class UInt128(val high: ULong, val low: ULong) : Comparable<UInt128>, Number() {
    ...
    infix fun minus(other: UInt128): UInt128 {
    ...
    private fun mulHelper(lhs:ULong,rhs:ULong): UInt128 {
    ...
    infix fun times(rhs:UInt128):UInt128{
        val i = mulHelper(low, rhs.low)
        val j = mulHelper(low, rhs.high)
        val k = mulHelper (high, rhs.low)
    ...
    fun shl(count: Int): UInt128 {
        val (h, l) = Int128(high, low).shl(count)
        return UInt128(h, l)
    }

    fun shr(count: Int): UInt128 {
    ...
    fun or(other: UInt128): UInt128 = UInt128(high or other.high, low or other.low)
